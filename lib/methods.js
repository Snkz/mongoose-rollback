function methods(schema) {
    ...
    schema.methods.getVersion = function(version, callback) {
        ...
        RollbackModel.findOne({"_id": id }, 
            ...
            function(err, hist) {
                ...
                if(!hist) {
                    // update history behind the scenes
                    //storeAsync(self);
                    //callback(err, [self.toObject()]);
                    storeAsync(self, callback);
                    return;
                }
        ...
        RollbackModel.findOne({"_id": id }, {"data": 1}, 
            function(err, hist) {
                ...
                if (!hist) {
                    ...
                    // async update that happens silently
                    //storeAsync(self);
                    //callback(err, [self.toObject()]);
                    storeAsync(self, callback);
                    return;
    ...
    function storeAsync(model, callback) {
        var id = model._id;
