...
function methods(schema) {
    ...
    //  Trim other revisions except this
    schema.methods.trim = function(version, callback) {
        var id = this._id;
        var self = this;
    
        if(isNaN(version)) {
            callback(new Error('version must be a number'));
            return;
        }
        ...
        RollbackModel.findOne({
            ...
            function(err, hist) {
                ...
                var prevModel = hist.data[0];
                if(typeof prevModel === 'undefined') {
                    return callback(new Error('Model at version ' + version + ' does not exist'));
                }
